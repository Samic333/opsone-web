...
function statusBadge(string $status): string {
    // Cockpit-light status mapping. Each status falls into exactly one of:
    //   cleared (green) | advisory (amber) | critical (red) | neutral (slate) | info (blue)
    // Spec: docs/opsone-brand-direction.md §3.
    static $cleared  = ['active','approved','published','completed','done','passed','online',
                        'acknowledged','signed','paid','open'];
    static $advisory = ['pending','in_review','review','awaiting','submitted','draft_review',
                        'requires_info','expiring','soon_due'];
    static $critical = ['suspended','rejected','revoked','expired','failed','blocked',
                        'aog','overdue','denied'];
    static $neutral  = ['inactive','draft','archived','closed','withdrawn','cancelled','offline'];

    $key = strtolower($status);
    if (in_array($key, $cleared,  true))  { $color = '#10b981'; } // --status-cleared
    elseif (in_array($key, $advisory, true)) { $color = '#f59e0b'; } // --status-advisory
    elseif (in_array($key, $critical, true)) { $color = '#ef4444'; } // --status-critical
    elseif (in_array($key, $neutral,  true)) { $color = '#7484a8'; } // muted slate
    else                                     { $color = '#3b82f6'; } // --status-info (default for unknown)
...
/**
 * Render an inline Heroicons-outline SVG icon for the sidebar and UI.
 * Usage: <?= sidebarIcon('chart-bar') ?>
 */
function sidebarIcon(string $name, int $size = 18): string {
    $paths = [
        ...
        'shield-exclamation'=> '<path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m0-10.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.75c0 5.592 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.75h-.152c-3.196 0-6.1-1.248-8.25-3.286Zm0 13.036h.008v.008H12v-.008Z"/>',
        'shield'            => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 2.25c-1.892 0-3.758.11-5.593.322C5.307 2.7 4.5 3.65 4.5 4.757V19.5a.75.75 0 0 0 1.041.694L12 18.075l6.459 2.119A.75.75 0 0 0 19.5 19.5V4.757c0-1.108-.806-2.057-1.907-2.185A48.507 48.507 0 0 0 12 2.25Z"/>',
        ...
        'question-circle'   => '<path stroke-linecap="round" stroke-linejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 5.25h.008v.008H12v-.008Z"/>',
 ...
 */
function opsoneLockup(string $size = 'md', string $node = '#06b6d4'): string {
    $markSizes = ['sm' => 16, 'md' => 22, 'lg' => 32, 'xl' => 48];
    $markSize = $markSizes[$size] ?? 22;
    return '<span class="opsone-lockup" style="display:inline-flex;align-items:center;gap:10px;">'
         . opsoneLogoMark($markSize, $node)
         . opsoneWordmark($size)
         . '</span>';
}
